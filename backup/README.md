...
| Fotos (actas, lecturas, novedades, perfiles de usuario) | `../minio_data/` (carpeta en la raíz del proyecto) | **No requiere generar nada** — MinIO ya escribe directo ahí. Solo hay que incluir esa carpeta en tu copia al disco externo. |
...
app medidores/minio_data/           (todas las fotos)
app medidores/.env                  (secretos — copia una vez, no hace falta todos los días)
```

## 2. Generar el backup de Postgres
...
## 3. Programado (ya está hecho, no hay que configurar nada)

Ya quedaron creadas dos tareas en el Programador de tareas de Windows:
...
robocopy "C:\Users\SERVIDOR\Documents\app medidores\minio_data" "D:\Backups\medidores\fotos" /MIR
```
